...
# @RegisterDocument('TL Script', ext=['.tlscript'])
# @RegisterContainer
# class TLExprDocument(TextDocument):
# 	"""docstring for Document"""
#
# 	def __typeCheckerInfo___(self):
# 		# giving the type checker a helping hand...
# 		super(TLExprDocument, self).__typeCheckerInfo___()
# 		self.filePath: FilePath = ''
# 		self.filePathForDisplay: str = ''
# 		self.fileName: str = ''
# 		self.fileLocationAbsolute: FilePath = ''
...
#
# 	filePath: FilePath = Serialized(default='', decorators=[
# 		pd.FilePath(filters=[('TL Expression', '.tlexpr')])
# 	])
#
# 	content: str = Serialized(default='elem -> recursion($elem, child -> $child.get(`tl.element:StructuredElement#parent`), 0, 100)')
#
# 	@utils.DeferredCallOnceMethod(delay=333)
# 	@utils.BusyIndicator
# 	def revalidateExpression(self) -> None:
# 		tree, errors = parseTLScript(self.content)
# 		if tree is not None:
# 			errors += tree.check([])
# 			self.expressionResultType = tree.returnType.typeStr
# 		self.errors = errors
#
# 	@content.onSet
# 	def content(self, newVal: str, oldVal: str) -> str:
# 		self.contentOnSet(newVal, oldVal)
# 		if newVal != oldVal:
# 			self.revalidateExpression()
# 		return newVal
#
# 	expressionResultType: str = Serialized(default='', shouldSerialize=False)
#
# 	encoding: str = Serialized(default='utf-8')
